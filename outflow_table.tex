...
\tablehead{\colhead{Source\tablenotemark{a}} & \colhead{R.A.} & \colhead{Decl.} & \colhead{$v_{\rm blue}$/$v_{\rm red}$\tablenotemark{b}} & \colhead{Confidence\tablenotemark{c}} & \colhead{Tanabe}\\
...
\tablenotetext{b}{These are the visually determined velocities \textbf{closest to the mean cloud velocity which most clearly separate the outflow lobes from the surrounding cloud}. Entries marked with '-' indicate that one of the outflow lobes is not detected. See Section~\ref{sec:identification} for details.}
